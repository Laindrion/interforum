...
button.forEach((btn, index) => {
  ...
  (function () {
  const modal = document.getElementById('org-structure-modal');
  const closeBtn = modal ? modal.querySelector('.close-btn') : null;
  const buttons = document.querySelectorAll('.org_structure-btn[data-unit]');
  ...
  const LOCALE = (document.querySelector('meta[name="app-locale"]')?.content || 'ru').toLowerCase();

  function stopScroll(){ try{ window.lenis?.stop?.(); }catch(e){} }
  function startScroll(){ try{ window.lenis?.start?.(); }catch(e){} }
  ...
  async function loadLang(lang){
    ...
    const res = await fetch(`/i18n/org-structure.${lang}.json`, { cache:'no-store' });
  ...
  async function openUnit(unit){
    const dataLang = await loadLang(LOCALE).catch(()=>null);
    let data = dataLang?.[unit];
    if(!data && LOCALE !== 'ru'){
      const ru = await loadLang('ru').catch(()=>null);
      data = ru?.[unit];
    }
    if(!data) return;
    ...
    contactsEl.innerHTML = Array.isArray(data.contacts)
      ? data.contacts.map(c => `<a href="${c.href || '#'}">${c.text || ''}</a>`).join('<br>')
      : '';

    modal.setAttribute('data-current-unit', unit);
    openModal();
  }
