...
$(document).ready(function () {
   ...
   const pageLang = document.documentElement.lang || "eng"; // Default to English if no lang attribute
   ...
   const translations = {
      ...
      uz: {
         ...
         second: "Biz haqimizda",
         third: "Batafsil",
      },
   };

   // Get the correct translations based on the page language
   const langTranslations = translations[pageLang] || translations.eng;

   // Dynamic pseudo-element content
   const style = document.createElement("style");
   style.textContent = `
          .circle__btn::before {
              content: "${langTranslations.default}";
          }
          .circle__btn.second::before {
              content: "${langTranslations.second}";
          }
          .circle__btn.third::before {
              content: "${langTranslations.third}";
          }
      `;
   document.head.appendChild(style);

   console.log("Pseudo-elements updated:");
   console.log(`Default button text: ${langTranslations.default}`);
   console.log(`Second button text: ${langTranslations.second}`);
   console.log(`Third button text: ${langTranslations.third}`);
   ...
   **********************/
   ...
   //Implementing
   let popupVideo = document.querySelectorAll(".popup-video");
   let galleryVideoItems = document.querySelectorAll(".gallery__video-item");
...
(function () {
   /**********************
**********************
   ORG STRUCTURE MODAL (a11y-safe)
*********************
**********************/
   const orgStructureModal = document.getElementById('org-structure-modal');
   const closeBtn = orgStructureModal ? orgStructureModal.querySelector('.close-btn') : null;
   const orgStructureBtns = document.querySelectorAll('.org_structure-btn[data-unit]');
   const page = document.querySelector('html');

   // elements to toggle content (title, body...)
   const titleEl = document.getElementById('org-modal-title');
   const subtitleEl = document.getElementById('org-modal-subtitle');
   const bodyEl = document.getElementById('org-modal-body');
   const contactsEl = document.getElementById('org-modal-contacts');

   // focus management
   let lastFocused = null;

   // make only the background inert (non-focusable) when modal is open
   function getBackgroundSiblings(modal) {
      const p = modal?.parentElement;
      if (!p) return [];
      return Array.from(p.children).filter(el => el !== modal);
   }
   const backgroundEls = getBackgroundSiblings(orgStructureModal);
   ...
   // lenis guards (optional)
   function stopScroll() { try { window.lenis && window.lenis.stop && window.lenis.stop(); } catch (e) { } }
   function startScroll() { try { window.lenis && window.lenis.start && window.lenis.start(); } catch (e) { } }
   ...
   function openModal() {
      ...
      // IMPORTANT: modal must NOT be aria-hidden when open
      ...
      // send focus into the dialog
      if (closeBtn) closeBtn.focus();
   }

   // CLOSE
   function closeModal() {
      // move focus out of modal BEFORE hiding it
      if (lastFocused && document.contains(lastFocused)) {
         lastFocused.focus();
      } else {
         // fallback: put focus on <body> to avoid leaving it inside a hidden region
         document.body.tabIndex = -1;
         document.body.focus();
         document.body.removeAttribute('tabindex');
      }

      // now it is safe to hide from AT
   ...
   // Wire clicks to OPEN (you already fill the content elsewhere)
   orgStructureBtns.forEach((btn) => {
   ...
   orgStructureModal.addEventListener('click', (event) => {
      if (event.target === orgStructureModal) closeModal();
   });
   ...
   document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && orgStructureModal.classList.contains('active')) {
         closeModal();
      }
   });
})();